...
def get_first_number_p2(line, rev=False):
    numbers =  {
        'one': 1,
        'two': 2,
        'six': 6,
    ...
    numbers.update({str(n): n for n in range(1,10)})
...
def part1(inpt):
    ...
    for line in inpt.split("\n"):
        num = get_first_number(line) * 10 + get_last_number(line)
        acc += num
    return acc
...
def part2(inpt):
    ...
    for line in inpt.split("\n"):
        try:
            n1 = get_first_number_p2(line)
            n2 = get_first_number_p2(line, rev=True)
            num = get_first_number_p2(line) * 10 + get_first_number_p2(line, rev=True)
        except TypeError:
...
7pqrstsixteen"""
    ...
    test_inpt3 = """hzfplpdt9"""

    real_inpt = """sdpgz3five4seven6fiveh
876mbxbrntsfm
