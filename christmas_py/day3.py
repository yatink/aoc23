...
class NumberSequence(object):
    def __init__(self, starting_digit):
        self.id = uuid.uuid1()
        self.str_digits = [starting_digit]

    def get_number(self):
        return int("".join(self.str_digits))

    def append_digit(self, digit):
        self.str_digits.append(digit)
...
def parse_input(inpt):
    ...
    numbers = {}
    symbols = {}
    parsing_number = None # If there's a number being currently parsed
    for y,line in enumerate(inpt.split("\n")):
        for x,ch in enumerate(line):
            if ch == '.':
                if parsing_number is not None:
                    parsing_number = None
            elif ch in digits:
                if not parsing_number:
                    parsing_number = NumberSequence(ch)
                else:
                    parsing_number.append_digit(ch)
                numbers[(x,y)]=parsing_number
            else:
                # It's a symbol
                if parsing_number:
                  parsing_number = None
...
def part1(inpt):
    symbols, numbers = parse_input(inpt)
    relevant_numbers = set()
    for k,v in symbols.items():
        for x,y in get_adjacent_indexes(*k):
            if (x,y) in numbers:
                relevant_numbers.add(numbers[(x,y)])
    return sum([rn.get_number() for rn in relevant_numbers])
...
def part2(inpt):
    ...
    for k,v in symbols.items():
        if v == "*":
            ...
            for x,y in get_adjacent_indexes(*k):
                if (x,y) in numbers:
                    adjacent_nums.add(numbers[(x,y)])
            if len(adjacent_nums) == 2:
                # It's a gear
                gear_ratios += reduce(mul, [an.get_number() for an in adjacent_nums])
...
.664.598.."""
    ...
    real_inpt = """............................................411.....................363..134.........463.775..........................506...................
