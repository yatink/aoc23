...
def parse_input(inpt):
    ...
    cube_pat = re.compile(r'^ (\d+) (blue|red|green)$')
    games = {}
    for line in inpt.split("\n"):
        game_id_str, rem = line.split(":")
        game_id = int(game_id_pat.match(game_id_str).groups()[0])
        sections = []
        for section in rem.split(";"):
            section_map = {}
            for cube_str in section.split(","):
                num, colour = cube_pat.match(cube_str).groups()
                section_map[colour] = int(num)
            sections.append(section_map)
        games[game_id] = sections
    return games


def get_adjacent_indexes(x,y):
    deltas = [1, 0, -1]
    return set(product([x+d for d in deltas], [y+d for d in deltas])) - {(x,y)}

                
def part1(inpt):
    games = parse_input(inpt)
    num_cubes = {'red': 12, 'green': 13, 'blue': 14}
    relevant_games ={
        g_id for g_id, sections in games.items() if all(num_cubes[cube] >= num for section in sections for cube, num in section.items())}
    ...
    for game_id, sets in games.items():
        ...
        for s in sets:
            for k,v in s.items():
                if v >= maxes[k]:
                    maxes[k] = v
        power_sum += reduce(mul, maxes.values())

    return power_sum

if __name__ == '__main__':
    test_inpt = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"""

    real_inpt = """Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
